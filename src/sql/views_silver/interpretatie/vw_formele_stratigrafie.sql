-- View: s_bodemenondergrond.vw_formele_stratigrafie_group_observations

-- DROP VIEW s_bodemenondergrond.vw_formele_stratigrafie_group_observations;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_formele_stratigrafie_group_observations
 AS
 WITH r AS (
         SELECT lip.permkey,
           ...
           FROM b_bodemenondergrond.tbl_interpretatie ti
             ...
             JOIN b_bodemenondergrond.tbl_formele_interpretatie tlb ON tlb.interpretatie_id = ti.interpretatie_id
        )
 SELECT (('observation:interp'::text || permkey) || '-'::text) || rn AS "@id",
    'qb:Observation'::text AS "@type",
    'dataset:formeleStratigrafie'::text AS "qb:dataSet",
    'cl-interpretatie:formeleStratigrafie'::text AS "dcterms:type",
    ...
    'cl-forme:'::text || lid1 AS "bodemenondergrond:bovengrens",
    'cl-forme:'::text || lid2 AS "bodemenondergrond:ondergrens",
        CASE
            WHEN lid1 = lid2 THEN NULL::text
            WHEN rel_lid1_lid2 = 'O'::bpchar THEN 'cl-releenh:of'::text
            WHEN rel_lid1_lid2 = 'T'::bpchar THEN 'cl-releenh:tot'::text
            WHEN rel_lid1_lid2 = 'E'::bpchar THEN 'cl-releenh:en'::text
            ELSE NULL::text
        END AS "bodemenondergrond:relatietusseneenheden",
    'cl-betr:'::text || betrouwbaarheid AS "ssn-system:qualityOfObservation"
   FROM r;

ALTER TABLE s_bodemenondergrond.vw_formele_stratigrafie_group_observations
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_formele_stratigrafie_group_observations_values

-- DROP VIEW s_bodemenondergrond.vw_formele_stratigrafie_group_observations_values;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_formele_stratigrafie_group_observations_values
 AS
 WITH r AS (
         SELECT lip.permkey,
            row_number() OVER (PARTITION BY lip.permkey ORDER BY tlb.van) AS rn,
...
ALTER TABLE s_bodemenondergrond.vw_formele_stratigrafie_group_observations_values
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_formele_stratigrafie_observation

-- DROP VIEW s_bodemenondergrond.vw_formele_stratigrafie_observation;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_formele_stratigrafie_observation
 AS
 WITH relevant_int AS (
         SELECT DISTINCT ti_1.interpretatie_id,
           ...
           FROM b_bodemenondergrond.tbl_interpretatie ti_1
             ...
             JOIN b_bodemenondergrond.tbl_formele_interpretatie tlb ON tlb.interpretatie_id = ti_1.interpretatie_id
        ), ti AS (
 ...
 SELECT DISTINCT 'interp:'::text || ti.relkey AS "@id",
    ...
    'cl-interpretatie:formeleStratigrafie'::text AS "dcterms:type",
    '"Formele stratigrafie op locatie van boorgat '::text || boring.permkey AS "rdfs:label",
    ti."sosa:resultTime",
...
ALTER TABLE s_bodemenondergrond.vw_formele_stratigrafie_observation
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_formele_stratigrafie_observation_group

-- DROP VIEW s_bodemenondergrond.vw_formele_stratigrafie_observation_group;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_formele_stratigrafie_observation_group
 AS
 WITH relevant_int AS (
 ...
 SELECT 'observationgroup:interp'::text || observations.relkey AS "@id",
    ...
    '"Observation group Formele stratigrafie op locatie van boorgat'::text || boring.permkey AS "rdfs:label",
...
ALTER TABLE s_bodemenondergrond.vw_formele_stratigrafie_observation_group
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_formele_stratigrafie_observation_list

-- DROP VIEW s_bodemenondergrond.vw_formele_stratigrafie_observation_list;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_formele_stratigrafie_observation_list
 AS
 WITH counter AS (
         SELECT lip.permkey,
...
ALTER TABLE s_bodemenondergrond.vw_formele_stratigrafie_observation_list
    OWNER TO postgres;
