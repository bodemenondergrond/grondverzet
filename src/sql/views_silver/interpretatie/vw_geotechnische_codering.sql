-- View: s_bodemenondergrond.vw_geotechnische_codering_group_observations

-- DROP VIEW s_bodemenondergrond.vw_geotechnische_codering_group_observations;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_geotechnische_codering_group_observations
 AS
 WITH r AS (
         SELECT lip.permkey,
            row_number() OVER (PARTITION BY lip.permkey ORDER BY tlb.van) AS rn,
            array_remove(ARRAY['cl-hoofdnaam:'::text || tlb.hoofdbenaming_1::text, 'cl-hoofdnaam:'::text || tlb.hoofdbenaming_2::text], NULL::text) AS "bodemenondergrond:hoofdnaam",
            array_remove(ARRAY[
                CASE
           ...
           FROM b_bodemenondergrond.tbl_interpretatie ti
             ...
             JOIN b_bodemenondergrond.tbl_geotechnische_codering tlb ON tlb.interpretatie_id = ti.interpretatie_id
        )
 SELECT (('observation:interp'::text || permkey) || '-'::text) || rn AS "@id",
    'qb:Observation'::text AS "@type",
    'dataset:informeleHydrostratigrafie'::text AS "qb:dataSet",
...
ALTER TABLE s_bodemenondergrond.vw_geotechnische_codering_group_observations
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_geotechnische_codering_group_observations_values

-- DROP VIEW s_bodemenondergrond.vw_geotechnische_codering_group_observations_values;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_geotechnische_codering_group_observations_values
 AS
 WITH r AS (
         SELECT lip.permkey,
            row_number() OVER (PARTITION BY lip.permkey ORDER BY tlb.van) AS rn,
...
ALTER TABLE s_bodemenondergrond.vw_geotechnische_codering_group_observations_values
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_geotechnische_codering_observation

-- DROP VIEW s_bodemenondergrond.vw_geotechnische_codering_observation;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_geotechnische_codering_observation
 AS
 WITH relevant_int AS (
         SELECT DISTINCT ti_1.interpretatie_id,
           ...
           FROM b_bodemenondergrond.tbl_interpretatie ti_1
             ...
             JOIN b_bodemenondergrond.tbl_geotechnische_codering tlb ON tlb.interpretatie_id = ti_1.interpretatie_id
        ), ti AS (
 ...
 SELECT DISTINCT 'interp:'::text || ti.relkey AS "@id",
    ...
    'cl-interpretatie:geotechnischeCodering'::text AS "dcterms:type",
    '"Geotechnische codering op locatie van boorgat '::text || boring.permkey AS "rdfs:label",
    ti."sosa:resultTime",
...
ALTER TABLE s_bodemenondergrond.vw_geotechnische_codering_observation
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_geotechnische_codering_observation_group

-- DROP VIEW s_bodemenondergrond.vw_geotechnische_codering_observation_group;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_geotechnische_codering_observation_group
 AS
 WITH relevant_int AS (
 ...
 SELECT 'observationgroup:interp'::text || observations.relkey AS "@id",
    ...
    '"Observation group geotechnische codering op locatie van boorgat '::text || boring.permkey AS "rdfs:label",
...
ALTER TABLE s_bodemenondergrond.vw_geotechnische_codering_observation_group
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_geotechnische_codering_observation_list

-- DROP VIEW s_bodemenondergrond.vw_geotechnische_codering_observation_list;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_geotechnische_codering_observation_list
 AS
 WITH counter AS (
         SELECT lip.permkey,
...
ALTER TABLE s_bodemenondergrond.vw_geotechnische_codering_observation_list
    OWNER TO postgres;
