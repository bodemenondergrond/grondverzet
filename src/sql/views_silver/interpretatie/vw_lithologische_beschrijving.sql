-- View: s_bodemenondergrond.vw_lithologische_beschrijving_group_observations

-- DROP VIEW s_bodemenondergrond.vw_lithologische_beschrijving_group_observations;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_lithologische_beschrijving_group_observations
 AS
 WITH r AS (
         SELECT lip.permkey,
            row_number() OVER (PARTITION BY lip.permkey ORDER BY tlb.van) AS rn,
            tlb.lithologische_beschrijving
           FROM b_bodemenondergrond.tbl_interpretatie ti
             ...
             JOIN b_bodemenondergrond.tbl_lithologische_beschrijving tlb ON tlb.interpretatie_id = ti.interpretatie_id
        )
 SELECT (('observation:interp'::text || permkey) || '-'::text) || rn AS "@id",
    'qb:Observation'::text AS "@type",
    'dataset:lithologischeBeschrijving'::text AS "qb:dataSet",
    'cl-interpretatie:lithologischeBeschrijving'::text AS "dcterms:type",
    ...
    lithologische_beschrijving AS "bodemenondergrond:beschrijving"
   FROM r;

ALTER TABLE s_bodemenondergrond.vw_lithologische_beschrijving_group_observations
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_lithologische_beschrijving_group_observations_values

-- DROP VIEW s_bodemenondergrond.vw_lithologische_beschrijving_group_observations_values;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_lithologische_beschrijving_group_observations_values
 AS
 WITH r AS (
         SELECT lip.permkey,
            row_number() OVER (PARTITION BY lip.permkey ORDER BY tlb.van) AS rn,
...
ALTER TABLE s_bodemenondergrond.vw_lithologische_beschrijving_group_observations_values
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_lithologische_beschrijving_observation

-- DROP VIEW s_bodemenondergrond.vw_lithologische_beschrijving_observation;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_lithologische_beschrijving_observation
 AS
 WITH relevant_int AS (
         ...
         SELECT DISTINCT relevant_int.relkey,
           ...
           FROM b_bodemenondergrond.tbl_interpretatie ti_1
             ...
             JOIN b_bodemenondergrond.lnk_id_permkey lip ON lip.id = ti_1.interpretatie_id AND lip.state = 'A'::text AND lip.type = 'INTERPRETATIE'::text
             JOIN b_bodemenondergrond.tbl_lithologische_beschrijving tlb ON tlb.interpretatie_id = ti_1.interpretatie_id
 ...
 SELECT DISTINCT 'interp:'::text || ti.relkey AS "@id",
    ...
    'cl-interpretatie:lithologischeBeschrijving'::text AS "dcterms:type",
    'Lithologische beschrijving op locatie van boorgat '::text || boring.permkey AS "rdfs:label",
    ti."sosa:resultTime",
...
ALTER TABLE s_bodemenondergrond.vw_lithologische_beschrijving_observation
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_lithologische_beschrijving_observation_group

-- DROP VIEW s_bodemenondergrond.vw_lithologische_beschrijving_observation_group;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_lithologische_beschrijving_observation_group
 AS
 WITH relevant_int AS (
 ...
 SELECT 'observationgroup:interp'::text || observations.relkey AS "@id",
    ...
    'Observation group Lithologische beschrijving op locatie van boorgat'::text || boring.permkey AS "rdfs:label",
...
ALTER TABLE s_bodemenondergrond.vw_lithologische_beschrijving_observation_group
    OWNER TO postgres;

-- View: s_bodemenondergrond.vw_lithologische_beschrijving_observation_list

-- DROP VIEW s_bodemenondergrond.vw_lithologische_beschrijving_observation_list;

CREATE OR REPLACE VIEW s_bodemenondergrond.vw_lithologische_beschrijving_observation_list
 AS
 WITH counter AS (
         SELECT lip.permkey,
...
ALTER TABLE s_bodemenondergrond.vw_lithologische_beschrijving_observation_list
    OWNER TO postgres;
